...
import { Navbar, Sidebar, Button, Avatar, Badge, Dropdown } from 'flowbite-react';
import {
  LayoutDashboard,
  AppWindow,
  Megaphone,
...
const Layout: React.FC<LayoutProps> = ({ children }) => {
  ...
  const navItems = [
    { name: t('common.dashboard'), path: '/dashboard', icon: <LayoutDashboard size={20} /> },
    { name: t('common.apps'), path: '/apps', icon: <AppWindow size={20} /> },
    { name: t('common.campaigns'), path: '/campaigns', icon: <Megaphone size={20} /> },
    { name: t('common.notifications'), path: '/notifications', icon: <Bell size={20} /> },
    { name: t('common.settings'), path: '/settings', icon: <Settings size={20} /> },
  ];
    ...
    <div className="flex flex-col min-h-screen">
      ...
      <Navbar fluid className="border-b border-gray-200 bg-white shadow-sm">
        <div className="flex items-center">
          <Button color="gray" className="mr-2 lg:hidden" onClick={toggleSidebar}>
            <Menu size={20} />
          </Button>
        ...
        <div className="flex items-center gap-2 md:gap-4">
          <Dropdown
            ...
            label={<Globe size={20} />}
            theme={{
              inlineWrapper: 'flex items-center py-2 px-1 text-gray-500 hover:text-gray-900'
            }}
          >
            <Dropdown.Header>
          ...
          >
            <Bell size={20} className='text-gray-500 hover:text-gray-900 ' />
            {unreadCount > 0 && (
              <Badge color="failure" className="absolute -top-1.5 -right-4">
                {unreadCount}
              </Badge>
            )}
          </div>
          ...
          <Dropdown
            ...
            theme={{
              inlineWrapper: 'flex items-center px-3 text-gray-500 hover:text-gray-900'
            }}
          >
            <Dropdown.Header>
            ...
            <div className="fixed inset-0 z-40 lg:hidden">
              ...
              <div className="relative flex flex-col w-80 max-w-sm h-full bg-white">
                <div className="flex items-center justify-between p-4 border-b border-gray-200">
                  <h2 className="text-xl font-semibold text-gray-800">Revenue-360</h2>
                  <Button color="gray" onClick={closeSidebar}>
                    <X size={20} />
                  </Button>
                </div>
