...
import TeamMembersModal from "../components/TeamMembersListComponent";
const Merchants: React.FC = () => {
  const { t } = useTranslation();
  const [merchantUsers, setMerchantUsers] = useState<IMerchant[]>([]);
    ...
    const fetchMerchantUsers = async () => {
      try {
        ...
        if (usersJson) {
          ...
          const merchants = allUsers.filter(
            (user: IMerchant) => user.role === "admin"
          );
          setMerchantUsers(merchants);
        }
      } catch (error) {
        console.error(
          "Error fetching merchant users from localStorage:",
          error
        );
      } finally {
        setLoading(false);
  ...
  const handleTeamMembersClick = (merchant: IMerchant) => {
    setIsModalOpen(true);
  ...
  return (
    <>
        ...
        <div className="overflow-x-auto border border-gray-200 dark:border-gray-700 rounded-md">
          <Table>
            <Table.Head className="border-b h-[50px] border-gray-200 dark:border-gray-700">
              ...
              <Table.HeadCell>{t("merchant.teamMembers")}</Table.HeadCell>
                ...
                >
                  ...
                  <Table.Cell>
                    <span
                      ...
                      onClick={() => handleTeamMembersClick(merchant)}
                    >
                      {merchant.teamMembers?.length || 0}
                    </span>
                  </Table.Cell>
                  <Table.Cell>
                    <span
                  ...
                  <Table.Cell>
                    <Tooltip content={t("merchant.view")} placement="bottom">
                      <Button color="light" size="xs" onClick={() => false}>
      ...
      <AnimatedModal
        ...
        title={t("merchant.teamMembers")}
        children={
          <TeamMembersModal teamMembers={selectedMerchant?.teamMembers || []} />
        }
        size="xl"
      />
    </>
